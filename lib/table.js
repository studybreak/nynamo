...
Table.prototype.deleteItem = function (key, callback) {

};


Table.prototype.getItem = function (key, consistent, callback) {
  ...
  var capacity = this.testCapacity('Read', item, consistent ? 1.0 : 0.5);
...
Table.prototype.changeItem = function (item, remove, callback) {
  ...
  // Store or delete the unparsed item so we remember its attribute types.
  if (remove) {
...
Table.prototype.testCapacity = function (type, item, multiplier) {
  var throughput = Item.capacity(item) * (multiplier || 1.0);
